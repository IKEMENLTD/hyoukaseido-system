...
export default async function QuantitativePage(props: QuantitativePageProps) {
  ...
  const { data: headCheck, error: headCheckErr } = await supabase
    .from('division_members')
    .select('is_head, division_id')
    .eq('member_id', currentMember.id)
    .eq('is_head', true);
  if (headCheckErr) console.error('[DB] division_members 取得エラー:', headCheckErr);
  ...
  if (existingScoresErr) console.error('[DB] eval_kpi_scores 取得エラー:', existingScoresErr);
