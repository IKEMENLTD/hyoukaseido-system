// =============================================================================
// Cron: 1on1ミーティングリマインダー
// 毎月1日 UTC 0:00 (JST 9:00) に実行
// 全アクティブ組織に対して通知
// =============================================================================
...
import { notifyOneOnOneReminder } from '@/lib/notifications/events';
...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const org of orgs) {
      try {
        await notifyOneOnOneReminder(org.id, supabase);
        results.push({ orgId: org.id, success: true });
      } catch (err: unknown) {
        const message = err instanceof Error ? err.message : '不明なエラー';
        console.error(
          `one-on-one-reminder送信失敗 (org: ${org.id}):`,
          message
        );
    ...
    const message = err instanceof Error ? err.message : '不明なエラー';
    console.error('one-on-one-reminder cron エラー:', message);
    return NextResponse.json(
